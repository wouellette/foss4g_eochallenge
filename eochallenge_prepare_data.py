...
from eolearn.io import ExportToTiff, S2L1CWCSInput
...
from sentinelhub import CRS, BBoxSplitter, DataSource, CustomUrlParam
# ML libraries
import joblib
...
args = easydict.EasyDict({
  ...
  "training_path": ["./training_data/trainingdata_germany_trimmed.geojson","./training_data/trainingdata_romania_trimmed.geojson"],
  "aoi_bufsize": 10000,
  "zoom_level": (20,20),
  "hour_diff": 73,
  "cloud_threshold": 0.4,
  "maxcc": 0.5,
  "dest": "/home/ubuntu/output/2018",
  "time_range": ["2018-01-01","2018-12-30"],
  "save_choice": True,
  "n_procs": 2,
  "resolution": 10,
...
class SentinelHubValidData:
    ...
    def __call__(self, eopatch):
        ...
        if 'CLM' in eopatch.mask:
            ...
            print("Cloud mask absent, return valid pixels only...")
...
def craft_input_features(eopatches, range_sample, lulc_classes, out_path=None, **kwargs):
    """
...
for idx, training in zip(country_list, trainings):
    ...
    for val in training_val:
        ...
        temp = temp.assign(area=temp['geometry'].to_crs({'init': 'epsg:3857'}).area / 1000000, split=0)
        total_area = temp['area'].sum()
        print(f'total km2 for class {val}: {total_area}')
    ...
    print("training pixel # :",sum(split_array[0]['geometry'].to_crs({'init': 'epsg:3857'}).area/100))
    print("test pixel # :",sum(split_array[1]['geometry'].to_crs({'init': 'epsg:3857'}).area/100))
...
def load_eopatch(bbox_splitter, time_interval, training_array, split_array, training_val, out_path, row, col, idx,
    ...
    while attempts < 2:
        ...
        custom_url_params={CustomUrlParam.EVALSCRIPT: custom_script_s2},  # custom url for 6 specific bands
...
def interpolate_eopatch(resample_range, training_val, out_path, idx,
    ...
    while attempts < 5:
        ...
        try:
            ...
            if 'LULC_SAMPLED' in patch_s2.mask_timeless:
                with open(f'{out_path}/lulc_sampled/range_sample.txt', "a") as f:
                    f.write('%s\n' % idx)
...
def batch_prepare(bbox_splitter, resample_range, time_range, training_array, split_array, training_val, row, col, out_path, idx):
    ...
    load_eopatch(bbox_splitter, time_range, training_array,
                 split_array, training_val, f'{out_path}', row, col, idx)
    interpolate_eopatch(resample_range, training_val, f'{out_path}', idx)
